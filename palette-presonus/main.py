...
def generate_seq_rgb(num1:int, num2:int, count:int) -> list[int]:
    rs = generate_seq_int(num1&0xff0000, num2&0xff0000, count)
    gs = generate_seq_int(num1&0xff00, num2&0xff00, count)
    bs = generate_seq_int(num1&0xff, num2&0xff, count)
    return [(0xff000000|(rs[i]<<8)|(gs[i]<<4)|bs[i]) for i in range(0, count)]
...
def main():
    ...
    for color1, color2 in zip(color_list[::2], color_list[1::2]):
        result.extend(generate_seq_rgb(int(color1), int(color2), args.count))
